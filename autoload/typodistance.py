...
# FYI: [Typo\-Distance/typodistance\.py at master · wsong/Typo\-Distance]
...
def getPossibleActions(s, layout='QWERTY'):
    ...
    for i in range(len(s)):
        ...
        for key in (sum([r for r in keyboardArray], []) +
                    sum([r for r in shiftedKeyboardArray], [])):
...
def typoGenerator(s, d, layout='QWERTY'):
    ...
    while(True):
        ...
        while True:
            ...
            while(c[t - 1] > c[t] + i):
                if r >= (actions[c[t] + i].cost(changedString) -
                         actions[c[t]].cost(changedString)):
...
# FYI: [python \- Determine prefix from a set of \(similar\) strings \- Stack Overflow]( https://stackoverflow.com/questions/6718196/determine-prefix-from-a-set-of-similar-strings )
...
def main():
    ...
    if len(sys.argv) == 1:
        for input in typoGenerator(base_str, 2):
            dist = (typoDistance(input, base_str) / len(base_str))
            common_prefix_length = len(commonprefix([base_str, input]))
            if dist < 0.8 and common_prefix_length >= 3:
                print(input, dist, common_prefix_length)
    else:
